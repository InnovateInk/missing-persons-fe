      ...
      <div className='w-full grid grid-cols-1 md:grid-cols-3  xl:grid-cols-4 gap-4 mt-16'>
        {data.map((item) => (
          <Card key={item.id} className='border-none shadow-none'>
            <Link href={`/${item.name.replaceAll(' ', '+')}`} className='w-full h-full'>
              <CardHeader className='p-0'>
                <div className='overflow-hidden relative '>
                  <Image
                    ...
                    height={350}
                    className={cn(
                      'h-[200px] w-full object-cover transition-all hover:scale-105 blur-none aspect-video  group-hover:opacity-50',
                      { 'blur-md': isLoading }
                    )}
                  />
                  <div className='absolute top-4 right-4 opacity-100 z-10'>
                    <Badge className='bg-[#B20000] text-background hover:bg-[#B20000] capitalize font-grotesk-roman px-4'>
                      {item.status}
                    </Badge>
                  </div>
                </div>
              ...
              <CardContent className='px-2 py-2'>
                <div className='flex justify-between  h-full items-center'>
                  ...
                  <Badge className='bg-[#FFE4E4] text-[#B20000] hover:bg-[#FFE4E4] h-full text-[10px] font-grotesk py-0.5 px-4'>
                    24 days ago
                  </Badge>
                </div>
              </CardContent>
